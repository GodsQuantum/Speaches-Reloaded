...
def _offset_result(result, offset_ms: int):
    segments = tuple(
...
class TranscribeCppDiarizationModelManager(BaseModelManager):
    ...
    def diarize(self, model_id: str, audio):
        import transcribe_cpp
...
class TranscribeCppModelManager(BaseModelManager):
    ...
    def handle_non_streaming_transcription_request(self, request, **_kwargs):
            ...
            with model.session() as session:
                if getattr(model, "arch", "") == "qwen3_asr" and request.audio.duration > 30.0:
                    result = run_qwen_chunked(
                        session, request.audio.data, sample_rate=request.audio.sample_rate,
        ...
        return transcribe_result_to_response(
            result, request.response_format, duration=request.audio.duration,
            language=language or request.language,
        )

    def handle_streaming_transcription_request(self, request, **_kwargs):
        ...
        with self.load_model(request.model) as model:
            ...
            emitted = ""
            with model.session() as session:
                with session.stream(**stream_kwargs) as stream:
    ...
    def handle_transcription_request(self, request, **kwargs):
        if request.stream:
...
def transcribe_result_to_response(result, response_format: str, *, duration: float, language: str | None = None):
    import openai.types.audio
    ...
    segments = [
        openai.types.audio.TranscriptionSegment(
            id=i, seek=0, start=seg.t0_ms / 1000, end=seg.t1_ms / 1000,
            text=seg.text, tokens=[], temperature=0.0, avg_logprob=0.0,
    ...
    words = [
        openai.types.audio.TranscriptionWord(
            start=word.t0_ms / 1000, end=word.t1_ms / 1000, word=word.text,
        )
    ...
    if response_format == "vtt":
        return "".join(
            format_as_vtt(seg.text, seg.t0_ms / 1000, seg.t1_ms / 1000, i)
            for i, seg in enumerate(result.segments)
        ), "text/vtt"
    if response_format == "srt":
        return "".join(
            format_as_srt(seg.text, seg.t0_ms / 1000, seg.t1_ms / 1000, i)
            for i, seg in enumerate(result.segments)
        ), "text/plain"
    raise ValueError(f"Unsupported response format: {response_format}")


def resolve_timestamp_mode(model, requested_granularities) -> str:
...
def resolve_language_hint(model, requested: str | None) -> str | None:
